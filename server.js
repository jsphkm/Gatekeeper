...
function gateKeeper(req, res, next) {
  ...
  let namepass = req.get('x-username-and-password');
  let parsed = queryString.parse(namepass);
  USERS.find(function(El){
    if (El.userName === parsed.user && El.password === parsed.pass){
      req.user = El;
      next();
    };
  });
